...
import type { float, integer } from "../Decoder.flow/Decoder.js"
export type { URL, Query, Push, float, integer }
...
class RouteParam<a> extends VariableRoute<a> /*implements QueryParam<a>*/ {
  paramDecoder: Decoder.Decoder<mixed, a>
  ...
  parseParam(input: string): ?a {
    const result = this.paramDecoder.decode(input)
...
export const param = <a>(
  decoder: Decoder.Decoder<mixed, a>,
  format: a => string
): Param<a> => new RouteParam(decoder, format)

export const String: Param<string> = param(Decoder.String, toString)
export const Integer: Param<Decoder.integer> = param(Decoder.Integer, toString)
export const Float: Param<Decoder.float> = param(Decoder.Float, toString)
