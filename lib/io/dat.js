...
/*::
import type { Archive, Select, ArchiveInfo, Timeout, Stat, DirectoryEntry, Encoding } from "../beaker/DatArchive.js"
export type { Archive, Select, ArchiveInfo, Timeout, Stat, DirectoryEntry, Encoding }
*/
...
export const createDirectory = future(async (
  url /*:URL*/,
  ...
  if (options && options.recursive) {
    return await createDirectoryTree(url, timeout)
  } else {
    const archive = await load(url)
    return await archive.mkdir(url.pathname, timeout)
  }
})

export const createDirectoryTree = future(async (
  url /*:URL*/,
  options /*::?:Timeout*/
) /*:Promise<void>*/ => {
  const archive = await load(url)
  const entries = url.pathname.split("/")
  // Get rid of trailing `/` in case it's present.
  ...
  while (entries.length > 1) {
    ...
    try {
      await archive.mkdir(path, options)
      break
    } catch (error) {
      if (error.name === "ParentFolderDoesntExistError") {
        stack.unshift(path)
        entries.pop()
      } else {
        throw error
