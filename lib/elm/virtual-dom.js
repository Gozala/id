...
export var keyedNodeNS = function(namespace, tag) {
  return function(factList, kidList) {
    for (
...
export function custom(factList, model, render, diff) {
  return {
    $: __2_CUSTOM,
    __facts: organizeFacts(factList),
    __model: model,
...
export var map = function(tagger, node) {
  return {
    $: __2_TAGGER,
    __tagger: tagger,
    __node: node,
...
export var attribute = function(key, value) {
  return new VirtualDOMAttribute(key, value)
}

export var attributeNS = function(namespace, key, value) {
  return {
    $: "a__1_ATTR_NS",
    key: key,
...
class TaggedEventHandler {
  ...
  decode(input) {
    ...
    if (result instanceof Error) {
      ...
      const { value, preventDefault, stopPropagation } = result
      return { value: this.tag(value), preventDefault, stopPropagation }
    }
  }
  equal(other) {
    return (
...
function makeCallback(eventNode, handler) {
  function callback(event) {
    ...
    var { value, stopPropagation, preventDefault } = result
...
export function patch(rootDomNode, oldVirtualNode, patches, eventNode) {
  if (patches.length === 0) {
    return rootDomNode
  }
